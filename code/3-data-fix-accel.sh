...
cp ${DATADIR}/original/accel/accel-${batch}*.txt ${DATADIR}/derived/accel

# BSD sed (on mac OS) requires a value for -i flag
sed -i '1d' ${DATADIR}/derived/accel/accel-${batch}*.txt
...
head -n1 ${DATADIR}/derived/accel/accel-${batch}*.txt | grep $header | wc -l
...
# remove those lines
sed -i '/NA\tNA\tNA/d' ${DATADIR}/derived/accel/accel-${batch}*.txt
...
awk -F'\t' '(NF!=8){print $0}' ${DATADIR}/derived/accel/accel-${batch}*.txt | sort -u
